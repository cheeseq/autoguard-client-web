<template>
  <div>
    <div class="row valign-wrapper">
      ...
      <div class="col s4">
        <div class="input-field" v-show="car.is_prepay">
          <input type="text" @input="onPrepaySumChange" :value="car.prepay_sum"
                 :class="{'invalid': $v.car.prepay_sum.$error}" placeholder="Сумма" required>
      ...
      <div class="col s5">
        <div class="input-field" v-show="car.is_prepay">
          ...
          <span v-if="!$v.car.prepay_expires_at.required" class="helper-text"
    ...
    <div class="row">
      <div class="col s4">
        <div class="input-field">
          <input type="text" id="car_manufacturer" v-model="$v.car.manufacturer.$model"
                 :class="{'invalid': $v.car.manufacturer.$error}" placeholder="Марка">
          <span v-if="!$v.car.manufacturer.required" class="helper-text" data-error="Обязательно для заполнения"></span>
        </div>
      </div>
      <div class="col s4">
        <div class="input-field">
          <input type="text" v-model="$v.car.model.$model" id="car_model" :class="{'invalid': $v.car.model.$error}"
                 placeholder="Модель">
      ...
      <div class="col s4">
        <div class="input-field">
          <input type="text" id="car_gov_id" v-model="$v.car.gov_id.$model" :class="{'invalid': $v.car.gov_id.$error}"
    ...
    <div class="row">
      <div class="col s8">
        <div class="input-field">
          <input type="text" id="car_owner_fullname" v-model="$v.car.owner_fullname.$model"
                 :class="{'invalid': $v.car.owner_fullname.$error}" placeholder="Ф.И.О. владельца">
          <span v-if="!$v.car.owner_fullname.required" class="helper-text"
                data-error="Обязательно для заполнения"></span>
        </div>
      </div>
      <div class="col s4">
...
export default {
  ...
  methods: {
    addCar() {
      this.$v.$touch();
      if (!this.$v.$invalid) {
        this.car.registered_at = new Date();
        this.car.expires_at = new Date().setMonth(new Date().getMonth() + 1);
        this.car.status = 1;
